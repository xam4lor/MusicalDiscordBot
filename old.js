...
const { prefix, discord_token, youtube_api_key } = require('./config.json');
...
client
   ...
   - **add** [*pos*] [*song*] : Add the song between the songs at *pos* and *pos+1*.
...
async function findSongsInList(songInfo, channel) {
    ...
    if (!playlistID || playlistID.length == 0) { // Is not a playlist
        ...
        let url = `https://www.googleapis.com/youtube/v3/playlistItems?playlistId=${playlistID[0].substring(5).replace("&", '')}&key=${youtube_api_key}&part=snippet&maxResults=50`;
